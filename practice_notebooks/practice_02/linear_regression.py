...
class MSELoss(LossFunction, LossFunctionClosedFormMixin):
    ...
    def _svd_analytic_solution(cls, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        d = X.shape[1]
        k = d - 1
        u, s, vt = scipy.sparse.linalg.svds(X, k=k, solver='arpack', tol = 0)
        s_inv = 1 / s
...
class LogCoshLoss(LossFunction):
    ...
    def gradient(self, X: np.ndarray, y: np.ndarray, w: np.ndarray) -> np.ndarray:
        diff = X @ w - y
        return (X.T @ np.tanh(diff)) / X.shape[0]
...
class HuberLoss(LossFunction):
    ...
    def loss(self, X: np.ndarray, y: np.ndarray, w: np.ndarray) -> float:
        ...
        quadr = 0.5 * (diff**2)
        liner = self.delta * (abs_diff - 0.5 * self.delta)
        return np.mean(np.where(abs_diff <= self.delta, quadr, liner))

    def gradient(self, X: np.ndarray, y: np.ndarray, w: np.ndarray) -> np.ndarray:
        diff = X @ w - y
        abs_diff = np.abs(diff)
        grad_a = np.where(abs_diff <= self.delta, diff, self.delta * np.sign(diff))
        return (X.T @ grad_a) / X.shape[0]